# functions to determine if columns contain latitude or longitude data
...
get_link_cols <- function(df) {
  logi_list <- sapply(df[1:100, ], function(a) {
    any(grepl("http|www|@", a), na.rm = TRUE)
  })
